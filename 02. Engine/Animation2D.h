...
public:
	...
	void SetNowFrameOffset(Vector2 offset);
...
public:
	void Create(const wstring& _strName, CResPtr<CTexture> _pTex, Vector2 _vLT, Vector2 _vCropSize, int _iFrmCount, float _fTerm);
	void Create(const wstring& _strName, CResPtr<CTexture> _pTex, Vector2 _vLT, Vector2 _vCropSize, Vector2 offset, int _iFrmCount, float _fTerm);
